...
function onDisConnect(result) {
    console.log('连接断开事件:onDisConnect');
    if (result.code == 0) {
        console.log("退出成功！");
    } else {
        if(anyChatVideoCallComponentInstance){
            console.log('网络连接异常，视频呼叫结束');
            anyChatVideoCallComponentInstance.destroyVideoCall();
        }
        SDKInstance.logout();
...
//呼叫目标用户回调
function onRequestVideoCallDone(result, data) {
    console.log('呼叫目标用户回调:onRequestVideoCallDone');
    console.log(result);
    $('#AnyChatVideoCallComponent').parent().show();	//显示组件容器
    // anyChatVideoCallComponentInstance.mTargetName = anyChatVideoCallComponentInstance.getFriendNickName(data.userId);
    // anyChatVideoCallComponentInstance.mTargetUserId = data.userId;
    // console.log(mTargetName)
    console.log("呼叫回调" + result.code)
    anyChatVideoCallComponentInstance.resolveErrorCode(result.code);


}
//接收视频呼叫请求通知
function onReceiveVideoCallRequest(data) {
    console.log('接收视频呼叫请求:onReceiveVideoCallRequest');
    console.log(data);
    if(data.userId == anyChatVideoCallComponentInstance.mSelfUserId){
        //自己呼自己，return
        return;
    }
    $('#AnyChatVideoCallComponent').parent().show();	//显示组件容器
    console.log(data.userStr);
    var receiveData = JSON.parse(data.userStr);
    var callType = '';
    var fromUserInfo = '';
    if(receiveData.hasOwnProperty('type')){
        anyChatVideoCallComponentInstance.calltype = receiveData.type;
        anyChatVideoCallComponentInstance.mTargetName = receiveData.videoCallUserInfo.nickName;
        anyChatVideoCallComponentInstance.mTargetUserId = receiveData.videoCallUserInfo.userId;
        callType = receiveData.type;
        fromUserInfo = receiveData.videoCallUserInfo;
    }else{
        console.log('请求信息格式错误');
        return;
    }
    ...
    if (callType == "video") {
        $("#getVideoRequest .requestText").text("用户【" + fromUserInfo.nickName + "】邀请您视频通话")
        $("#requestVideo").hide()
        $("#getVideoRequest").show()
        $("#acceptVideoBtn").unbind().click(function() {
            console.log("同意视频请求")
            // var list = getRoomUserNum()
            anyChatVideoCallComponentInstance.acceptVideoCall(data.userId)
        })
        $("#rejectVideoBtn").unbind().click(function() {
            console.log("拒绝视频请求")
            anyChatVideoCallComponentInstance.rejectVideoCall(data.userId)
        })
    } else {
        $("#getAudioRequest .requestText").text("用户【" + fromUserInfo.nickName + "】邀请您语音通话")
        $("#requestAudio").hide()
        $("#getAudioRequest").show()
        $("#acceptAudioBtn").unbind().click(function() {
            console.log("同意语音请求")
            // var list = getRoomUserNum()
            anyChatVideoCallComponentInstance.acceptVideoCall(data.userId)
        })
        $("#rejectAudioBtn").unbind().click(function() {
            console.log("拒绝语音请求")
            anyChatVideoCallComponentInstance.rejectVideoCall(data.userId)
        })
    }
...
function onReceiveVideoCallStart(data) {
    console.log('接收视频呼叫开始通知:onReceiveVideoCallStart');
    console.log(data);
    // console.log("呼叫类型：" + data.userStr)
    anyChatVideoCallComponentInstance.roomId = data.roomId;
    // console.log(calltype);
    anyChatVideoCallComponentInstance.enterRoomRequest(anyChatVideoCallComponentInstance.roomId)
...
function onReceiveVideoCallError(result) {
    console.log('接收视频呼叫异常通知:onReceiveVideoCallError');
    console.log(result);
    anyChatVideoCallComponentInstance.resolveErrorCode(result.code);
}
...
function onReceiveVideoCallFinish(data) {
    console.log('接收视频呼叫结束通知:onReceiveVideoCallFinish');
    console.log(data)
    if (data.code == 0) {

        var errorCode = anyChatVideoCallComponentInstance.instance.leaveRoom();
        console.log("退出房间" + errorCode);
        if (anyChatVideoCallComponentInstance.timer) {
            anyChatVideoCallComponentInstance.timer.stop()
        }
        // var logoutCode2 = anyChatVideoCallComponentInstance.logout();
        // var logoutCode = this.SDKInstance.logout();
        // console.log(logoutCode2, 'logoutCode2')
        ...
        // }, 2000)
        console.log("呼叫结束");
        if(anyChatVideoCallComponentInstance.isError){
            anyChatVideoCallComponentInstance.isError = false;
            return;
        }
        $(".middle").text("通话已结束");
        $("#requestVideo").hide();
        ...
        // }, 2000)
        console.log("呼叫结束")
        if(anyChatVideoCallComponentInstance.isReject){
            anyChatVideoCallComponentInstance.isReject = false;
            return;
        }
...
//用户进出房间
function onRoomUserInAndOut(data) {
    console.log('用户进出房间:onRoomUserInAndOut');
    console.log(data)
    // data.action : 0表示进入房间，1表示退出房间
    if(data.action == 1){
        anyChatVideoCallComponentInstance.getRemoteVideo();
        anyChatVideoCallComponentInstance.getRemoteAudio();
    }else{
        // console.log('对方异常退出，视频呼叫结束');
        // anyChatVideoCallComponentInstance.destroyVideoCall();
    }

}
//进入视频房间回调
function onAnyChatEnterRoomForVideo(result, data) {
    console.log('视频房间回调:onAnyChatEnterRoomForVideo');
    console.log(result)
    // console.log(calltype)
    // DisplayLoadingDiv(false);
    if (result.code == 0) {
        ...
        $(".alert").hide();
        //定时器初始化
        anyChatVideoCallComponentInstance.timer = new timerTool()
        //开始计时
        anyChatVideoCallComponentInstance.timer.begin('video_time')
        anyChatVideoCallComponentInstance.cameras = anyChatVideoCallComponentInstance.getLocalCameras();
        anyChatVideoCallComponentInstance.microphones = anyChatVideoCallComponentInstance.getLocalMicrophones();
        // anyChatVideoCallComponentInstance.speaks = anyChatVideoCallComponentInstance.getLocalSpeaks();
        anyChatVideoCallComponentInstance.getRemoteVideo();
        anyChatVideoCallComponentInstance.getRemoteAudio();
        anyChatVideoCallComponentInstance.openCamera(0);
        anyChatVideoCallComponentInstance.openMicrophone(0);
...
//进入语音房间回调
function onAnyChatEnterRoomForAudio(result, data) {
    console.log('进入语音房间回调:onAnyChatEnterRoomForAudio');
    console.log(result);
    if (result.code == 0) {
        // $("#audio_text").text(anyChatVideoCallComponentInstance.mTargetName)
        $("#requestVideo").hide();
        $("#requestAudio").hide();
        $("#getVideoRequest").hide();
        $("#getAudioRequest").hide();
        ...
        anyChatVideoCallComponentInstance.timer = new timerTool();
        //开始计时
        anyChatVideoCallComponentInstance.timer.begin('audio_time');
        anyChatVideoCallComponentInstance.microphones = anyChatVideoCallComponentInstance.getLocalMicrophones();
        // anyChatVideoCallComponentInstance.speaks = anyChatVideoCallComponentInstance.getLocalSpeaks();
        anyChatVideoCallComponentInstance.openMicrophone(0);
        anyChatVideoCallComponentInstance.getRemoteAudio();
