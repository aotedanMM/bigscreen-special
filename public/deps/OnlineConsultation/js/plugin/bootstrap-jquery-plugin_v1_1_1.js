  ...
  $.fn.dialog = function(options) {
    ...
    var create = function() {
      ...
      var msghtml
        ...
        + '<iframe id="modal_iframe" frameborder="0" scrolling="no" style="left: 0px; top: 0px; '
    ...
    //bug: fixed by su.wenrong add callback
    var close = function(destroy,callback,data) {
      // call the bootstrap modal to handle the hide events and remove msgbox after the modal is hidden
      $msgbox.one('hidden.bs.modal',data,function(e) {
          if (destroy) {
    ...
    if (options == "destroy") {
      //bug: fixed by su.wenrong add callback
      //close(true);
       $.isFunction(arg1) ? (arg2 ? close(true,arg1,arg2) :close(true,arg1)):close(true);
    }
...
$.messager = (function() {
  var alert = function(title, message,callback) {
    ...
    $("<div>" + message + "</div>").dialog({
      ...
      , buttons: [{
          ...
          , click: function() {
              $(this).dialog("destroy",callback);
            }
        }]
    });
  };
  var confirm = function(title, message, callback,parmData,buttons) {
    var model = $.messager.model;
    var okText ='';
    var cancelText = '';
    if(buttons === void 0){
      okText = model.ok.text;
      cancelText = model.cancel.text;
    }else{
      okText = buttons.ok;
    ...
    $("<div>" + message + "</div>").dialog({
      ...
      , buttons: [{
          ...
          , click: function() {
        	  //bug: fixed by su.wenrong add callback
              $(this).dialog("destroy",callback,parmData);
              //callback && callback();
            }
        },
        {
  ...
  var prompt = function(title, message, callback,parmData,defaultValue,rule) {
    var model = $.messager.model;
    if(defaultValue === void 0){
      defaultValue = '';
    }
    $("<div>" + message + "<input type='text' id = 'promptInput' class='form-control' value = '"+ defaultValue+"'/></div>").dialog({
        title:   title
        // override destroy methods;
      , onClose: function() {
          $(this).dialog("destroy",callback(null));
        }
      , buttons: [{
            text: model.ok.text
          , classed: model.ok.classed || "btn-success"
          , click: function() {
            //bug: fixed by su.wenrong add callback
                //rule为制定的提示框点击确定时值的的检验规则
                if(typeof (rule) === "function"){
                    if(rule($('#promptInput').val())){
                      //值符合规则才执行回调，否则在规则中提示值的不合法原因
                        $(this).dialog("destroy",callback($('#promptInput').val()),parmData);
                    }
                }else{
                    $(this).dialog("destroy",callback($('#promptInput').val()),parmData);
                }
