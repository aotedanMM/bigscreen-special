...
export const disasterStatisticsMap: any = {
    resetData: '',
    // 初始化方法
    ...
    async loadData(cb: any , index?: any) {
        const that: any = this;
        await gisStaticServer.getDisasterStatistics().then((data: any) => {
            that.disasterDecideData = data;
            disasterStatisticsMap.resetData = data;
            cb(data.list , index);
            disasterStatisticsMap.vuexDataLoad(data.list, data.vuexList, that);
            disasterStatisticsMap.vuexcircleFlag(data.list, that);
        });
    },
    vuexDataLoad(data: any, key: any, self: any) {
        ...
        // 主动调用向nva 发送的事件
        disasterStatisticsMap.sendNvaTotal(data, self);
    },
    ...
    sendNvaTotal(data: any, self: any) {
        ...
        self.messsageBus.emit('nvaTotal', tempTotal);
    },
    // 重置数据的方法
    resetThisData(slef: any) {
        disasterStatisticsMap.vuexDataLoad(disasterStatisticsMap.resetData.list , disasterStatisticsMap.resetData.vuexList , slef);
    },
    // 切换2d、3d的方法
