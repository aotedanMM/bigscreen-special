...
<script lang="ts">
  ...
  export default class PowerfacilitiesList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '供电设施列表';
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('powerfacilities', item.id);
      }, 500);

      // this.getComponent().openPopup('powerfacilities', item.id);
