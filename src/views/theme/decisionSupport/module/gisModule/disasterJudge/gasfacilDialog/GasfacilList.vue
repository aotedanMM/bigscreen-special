...
<script lang="ts">
  ...
  export default class GasfacilList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '燃气供应设施列表';
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('gasfacil', item.id);
      }, 500);

      // this.getComponent().openPopup('gasfacil', item.id);
