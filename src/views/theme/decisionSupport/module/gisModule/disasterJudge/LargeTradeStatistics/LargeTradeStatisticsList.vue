...
<script lang="ts">
  ...
  export default class LargeTradeStatisticsList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '大型商贸列表';
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('market', item.id);
      }, 500);

      // this.getComponent().openPopup('market', item.id);
