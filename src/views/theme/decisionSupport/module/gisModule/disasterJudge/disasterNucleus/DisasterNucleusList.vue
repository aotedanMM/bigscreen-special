...
<script lang="ts">
  ...
  export default class DisasterNucleusList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '核设施列表';
...
// //数据源
    ...
    private FnSourceTypeCode() {
      this.sourceTypeCodeObj = dataSourceConfig('DisasterNucleusList');
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('nuclear', item.id);
      }, 500);

      // this.getComponent().openPopup('nuclear', item.id);
