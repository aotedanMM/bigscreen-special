...
export default class FIREWORKWAREHOUSEList extends Vue {
  //  private listDataFlag  = false;
  public title: any = '烟花爆竹仓库列表';
  ...
  private FnSourceTypeCode() {
    this.sourceTypeCodeObj = dataSourceConfig('DisasterFireworksList');
  ...
  private clickItemEvent(item: any) {
    ...
    setTimeout(() => {
      this.getComponent_new().locationCenter('fireworkhouse', item.id);
    }, 500);

    // this.getComponent().openPopup('explosive', item.id);
