<template>
  <div class="DisasterView">
    ...
    >
      <template v-slot:title>烟花爆竹仓库分布</template>
      <template v-slot:unit>个</template>
    </MapDialog>
  </div>
</template>
<script lang="ts">
...
@Component({
    name: 'FIREWORKWAREHOUSE',
    components: {
      MapDialog,
    },
    mixins: [MapCommon],
  })
  export default class FIREWORKWAREHOUSE extends Vue {
    ...
    // gis方法param
    private moduleType: any = 'fireworkhouse';
    ...
    private init() {
      ...
      if (this.$store.state.eventPushStore.eventLocation.EventType && this.showInit) {
        this.getComponent_new().getMultiuleOneNum(this.moduleType).then((dataTemp: any) => {
          const data: any = [];
          ...
          if (this.$store.state.dataFilterControl.zhypGeoType.key === 'jyqYp') {
            ...
            if (i === data.length) {
              ...
              retIsChecked = true;
              newArr = item.list;
              this.getComponent_new()._showPoint(item.list, this.moduleType, '');
        ...
        // 添加标点
 // 添加标点
        this.getComponent_new().getMultiuleOneNum(this.moduleType, jsonObj ? jsonObj : null).then((dataObj: any) => {
        const data = {data: [dataObj], total: dataObj.length};
          // this.getComponent().addResource(dataObj);
        this.viewResData = data;

        this.getComponent_new()._showPoint(dataObj, this.moduleType, '');
        this.messsageBus.emit('listSearchObj', dataObj);
      });
