...
<script lang="ts">
  ...
  export default class DisasterPlaneList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '机场列表';
...
// //数据源
    ...
    private FnSourceTypeCode() {
      this.sourceTypeCodeObj = dataSourceConfig('DisasterPlaneList');
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('airport', item.id);
      }, 500);
      // this.getComponent().openPopup('airport' , item.id);
    }
