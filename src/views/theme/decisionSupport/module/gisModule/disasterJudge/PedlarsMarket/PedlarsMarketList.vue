...
<script lang="ts">
  ...
  export default class PedlarsMarketList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '集贸市场列表';
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('bazaar', item.id);
      }, 500);

      // this.getComponent().openPopup('bazaar', item.id);
