...
<script lang="ts">
  ...
  export default class BigGasfacilList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '大型能源供应设施列表';
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('powerfacil', item.id);
      }, 500);
      // this.getComponent().openPopup('powerfacil', item.id);
