...
<script lang="ts">
  ...
  export default class DisasterCoalList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '煤矿企业列表';
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('coalMine', item.id);
      }, 500);
      // this.getComponent().openPopup('coalMine', item.id);
