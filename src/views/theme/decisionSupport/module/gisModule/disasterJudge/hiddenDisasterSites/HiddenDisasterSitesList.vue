...
<script lang="ts">
  ...
  export default class HiddenDisasterSitesList extends Vue {
    ...
    private FnSourceTypeCode() {
      this.sourceTypeCodeObj = dataSourceConfig('HiddenDisasterSitesList');
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter(this.$store.state.gisModuleDisasterJudge.disasterJudgeType, item.id);
      }, 500);

      // this.getComponent().openPopup(this.$store.state.gisModuleDisasterJudge.disasterJudgeType, item.id);
