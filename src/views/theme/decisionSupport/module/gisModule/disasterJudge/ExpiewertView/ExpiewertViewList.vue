...
<script lang="ts">
  ...
  export default class ExpiewertViewList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '专家列表';
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('emergenceexpert', item.id);
      }, 500);

      // this.getComponent().openPopup('emergenceexpert', item.id);
