...
<script lang="ts">
  ...
  export default class DisaterSchoolList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '学校列表';
    private filexed = ['num', 'name', 'typecodeName', 'level', '_distance'];
...
// //数据源
    ...
    private FnSourceTypeCode() {
      this.sourceTypeCodeObj = dataSourceConfig('DisaterSchoolList');
      ...
      this.messsageBus.on('listSearchObj', (item: any) => {
        ...
        for (let index = 0; index < item.length; index++) {
          ...
          const data = {
            ...
            typecodeName: element.schooltypename ? element.schooltypename : '- -',
          };
          listData.push(data);
        }
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('bas_school', item.id);
      }, 500);
      // this.getComponent().openPopup('bas_school' , item.id);
    }
    ...
    private mounted() {
      (this as any).resolveMap.call(this, 'map').then(() => {
        // this.getComponent().off('EventPointspopup');
        this.getComponent().on('EventPointspopup', this.onShowPopup, this);
        // this.getComponent().off('disasterXZDist'); // 替换 列表数据 的监听
