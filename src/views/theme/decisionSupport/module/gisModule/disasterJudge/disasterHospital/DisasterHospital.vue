<template>
    <div class="DisasterView DisasterHospital">
        <MapDialog
                :viewData="viewData"
                :viewResData="viewResData"
                :moduleType="moduleType"
        >
            <template v-slot:title>医院分布</template>
...
<script lang="ts">
  ...
  @Component({
    name: 'DisasterHospital',
    components: {
      MapDialog,
    },
    mixins: [MapCommon],
  })
  export default class DisasterHospital extends Vue {
    ...
    private moduleType: any = 'hospital';
    ...
    private watchGetData() {
      this.viewData.forEach((i: any, ind: any, data: any) => {
        this.getComponent()._clearLayerByID(this.moduleType);
      });
