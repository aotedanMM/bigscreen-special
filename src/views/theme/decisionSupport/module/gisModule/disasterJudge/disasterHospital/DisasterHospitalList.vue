...
<script lang="ts">
  ...
  export default class DisasterHospitalList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '医院列表';
    private filexed = ['num', 'name', 'orgtypename'];
...
// //数据源
    ...
    private FnSourceTypeCode() {
      this.sourceTypeCodeObj = dataSourceConfig('DisasterHospitalList');
    ...
    private FnListData() {
      this.messsageBus.on('listSearchObj', (item: any) => {
        ...
        for (let index = 0; index < item.length; index++) {
          ...
          if (element.orgtypename !== null) {
            orgtypename = element.orgtypename;
          }
          const data = {
            name: Letname,
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('hospital', item.id);
      }, 500);
      // this.getComponent_new().openPopup('hospital', item.id);
    }
    ...
    private mounted() {
      (this as any).resolveMap.call(this, 'map').then(() => {
        this.getComponent_new().on('EventPointspopup', this.onShowPopup, this);
        this.getComponent_new().on('disasterXZDist', this.FnShowXZD, this); // 替换 列表数据 的监听
      });
    }
