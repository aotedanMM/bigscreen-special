...
export default class WatersourceAirPlaneList extends Vue {
  //  private listDataFlag  = false;
  public title: any = '直升机取水点列表';
  ...
  private FnSourceTypeCode() {
    // debugger
    this.sourceTypeCodeObj = dataSourceConfig('DisasterReservoirList');
    dataSourcesServer
  ...
  private clickItemEvent(item: any) {
    ...
    // this.getComponent().openPopup('watersource_air', item.id);
    setTimeout(() => {
        this.getComponent_new().locationCenter('for_watersource', item.id);
      }, 500);
  }
  private FnHoverItemEvent(item: any) {
    ...
    this.getComponent1().addHover(
      'disaster_judge_resource__watersource_air__0',
      item.id,
    );
  }
