...
export default class WatersourcePufPlaneList extends Vue {
  //  private listDataFlag  = false;
  public title: any = '远程输水取水码头列表';
  ...
  private clickItemEvent(item: any) {
    ...
    setTimeout(() => {
        this.getComponent_new().locationCenter('for_waterport', item.id);
      }, 500);
  }
  private FnHoverItemEvent(item: any) {
    ...
    this.getComponent1().addHover(
      'disaster_judge_resource__watersource_puf__0',
      item.id,
    );
  }
