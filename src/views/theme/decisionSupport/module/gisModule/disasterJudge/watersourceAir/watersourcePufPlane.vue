<template>
  <div class="DisasterView DisasterMessenger">
    ...
    >
      <template v-slot:title>取水点加取水码头</template>
...
@Component({
  name: 'WatersourcePlane',
  components: {
    MapDialog,
  },
  mixins: [MapCommon],
})
export default class WatersourcePufPlane extends Vue {
  ...
  // gis方法param
  private moduleType: any = 'watersource_puf';
  ...
  private setGeometryFn(newVal: any, oldVal: any) {
    if (this.$store.state.dataFilterControl.filter.geometry && this.showInit) {
      this.showInit = false;
    } else if (
  ...
  private created() {
    ...
    this.$store.commit('gisModuleDisasterJudge/setDisasterJudgeType', this.moduleType);
      ...
      component.load([this.moduleType]).then((data: any) => {
        ...
        this.viewData = this.viewResData.data.map((item: any) => {
          ...
          if (this.$store.state.controlMoudle.mapCircleQueryType === 0) {
            ...
            // 判断是否是烈度圈
            //  添加高亮状态
            retIsChecked = this.tabList[i - 1].checked;
            if (this.tabList[i - 1].checked) {
              newArr = newArr.concat(item.list);
              component.showResource(this.moduleType, [item.level]);
            }
          }
      ...
      installDisasterJudgeServer.hazServer
        ...
        .then((dataObj: any) => {
          ...
          this.viewData = this.viewResData.data.map((item: any) => {
            ...
            this.messsageBus.emit('listSearchObj', newArr);
            return {
              levelTitle: populationSetUnit(0, 0),
