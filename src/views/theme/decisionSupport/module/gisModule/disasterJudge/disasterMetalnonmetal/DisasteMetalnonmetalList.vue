...
<script lang="ts">
  ...
  export default class DisasteMetalnonmetalList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '非煤矿山列表';
    ...
    private FnSourceTypeCode() {
      this.sourceTypeCodeObj = dataSourceConfig('DisasterCoalList');
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('metalnonmetal', item.id);
      }, 500);
      // this.getComponent().openPopup('metalnonmetal', item.id);
