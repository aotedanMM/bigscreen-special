...
<script lang="ts">
  ...
  export default class DisasterReservoirList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '水库大坝列表';
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_newForLeft().locationCenter('reservoir', item.id);
      }, 500);

      // this.getComponent().openPopup('reservoir', item.id);
    ...
    //  地图组件(水库)
    private getComponent_newForLeft() {
      const factory = this.$ioc.resolve('GISFactory-map');
