...
@Component({
  name: 'DisasterMajorDangerList',
  mixins: [MapCommon],
  components: {
    AreaList,
  },
})
export default class DisasterMajorDangerList extends Vue {
  public title: any = '重大危险源列表';
  ...
  private FnSourceTypeCode() {
    // debugger
    this.sourceTypeCodeObj = dataSourceConfig('DisasterMajorDangerList');
    dataSourcesServer.getLimitDataList({typeCode: this.sourceTypeCodeObj.typeCode}).then((data: any) => {
      const res = data;
      this.sourceTypeCodeObj.sourceData = res.data[0];
      console.log(this.sourceTypeCodeObj, 'this.sourceTypeCodeOb111111111111111111111');
      console.log( res.data, 'res.data222222222222222222');
    });
  }
    ...
    this.messsageBus.on('listSearchObj', (item: any) => {
      ...
      for (let index = 0; index < item.length; index++) {
        ...
        if (element.safetylevel !== undefined && element.safetylevel !== null) {
          const safetylevelInfo: any = this.SelectClass.find((v: any) => v.value === element.safetylevel);
          if (safetylevelInfo) {
            Letsafetylevel = safetylevelInfo.label;
          }
          typecode = element.safetylevel;
        } else {
          typecode = element.dangerlevel;
          Letsafetylevel = element.dangerlevelname;
        }
        ...
        const data = {
          ...
          safetylevel: element.MAJORDANGERLEVELCODE ? this.levelData[element.MAJORDANGERLEVELCODE * 1 - 1] : '- -',
        };
        listData.push(data);
      }
  ...
  private clickItemEvent(item: any) {
    ...
    setTimeout(() => {
      this.getComponent_new().locationCenter('majorDanger', item.id);
    }, 500);
    // this.getComponent().openPopup('majorDanger', item.id);
  }

  private FnHoverItemEvent(item: any) {
    const type = 'disaster_judge_resource__' +
