...
<script lang="ts">
  ...
  export default class ShelterViewList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '避难场所列表';
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('shelter', item.id);
      }, 500);


      // this.getComponent().openPopup('shelter', item.id);
