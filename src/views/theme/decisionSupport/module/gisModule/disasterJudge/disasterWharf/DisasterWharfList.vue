...
<script lang="ts">
  ...
  export default class DisasterWharfList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '码头列表';
    ...
    private FnSourceTypeCode() {
      this.sourceTypeCodeObj = dataSourceConfig('DisasterWharfList');
      dataSourcesServer
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('portwharf', item.id);
      }, 500);

      // this.getComponent().openPopup('portwharf', item.id);
