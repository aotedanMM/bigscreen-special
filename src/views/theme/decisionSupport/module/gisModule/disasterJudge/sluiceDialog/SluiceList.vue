...
<script lang="ts">
  ...
  export default class SluiceList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '水闸工程列表';
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('sluice', item.id);
      }, 500);

      // this.getComponent().openPopup('sluice', item.id);
