<template>
    <div class="DisasterView">
        ...
        >
            <template v-slot:title>危化企业分布</template>
...
<script lang="ts">
  ...
  @Component({
    name: 'DisasterCompany',
    components: {
      MapDialog,
    },
    mixins: [MapCommon],
  })
  export default class DisasterCompany extends Vue {
    ...
    // gis方法param
    private moduleType: any = 'hazardous';
    ...
    private created() {
      if (this.$store.state.dataFilterControl.filter.geometry || this.$store.state.dataFilterControl.zhypGeoType.key === 'searchYp') { // 看空间字段是否有值
        if (this.$store.state.mapTools.nearbyQueryVisible) { // 确认周边查询是否打开初始化进来的时候
          this.showInit = false;
        } else {
          this.showInit = true;
        }
