...
<script lang="ts">
  ...
  export default class IndustryViewList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '工贸企业列表';
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('metallurgical', item.id);
      }, 500);

      // this.getComponent().openPopup('metallurgical', item.id);
