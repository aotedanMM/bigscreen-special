...
<script lang="ts">
  ...
  export default class SupwatfacilList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '供水设施列表';
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('supwatfacil', item.id);
      }, 500);

      // this.getComponent().openPopup('supwatfacil', item.id);
