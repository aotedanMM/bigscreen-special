...
<script lang="ts">
  ...
  @Component({
    name: 'LargeCulturalAndSportsList',
    mixins: [MapCommon],
    components: {
      //   DistrictRightModule,
      AreaList,
  ...
  export default class LargeCulturalAndSportsList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '大型文化体育场所列表';
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('Gymnasium', item.id);
      }, 500);

      // this.getComponent().openPopup('Gymnasium', item.id);
