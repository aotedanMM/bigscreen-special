...
<script lang="ts">
  ...
  export default class DisasterTrainList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '火车站列表';
...
// //数据源
    ...
    private FnSourceTypeCode() {
      this.sourceTypeCodeObj = dataSourceConfig('DisasterTrainList');
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('railwaystation', item.id);
      }, 500);

      // this.getComponent().openPopup('railwaystation', item.id);
