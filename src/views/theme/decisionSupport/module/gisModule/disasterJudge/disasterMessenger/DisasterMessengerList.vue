...
<script lang="ts">
  ...
  export default class DisasterMessengerList extends Vue {
    //  private listDataFlag  = false;
    public title: any = '灾情信息员列表';
...
// //数据源
    ...
    private FnSourceTypeCode() {
      this.sourceTypeCodeObj = dataSourceConfig('DisasterMessengerList');
    ...
    private clickItemEvent(item: any) {
      ...
      setTimeout(() => {
        this.getComponent_new().locationCenter('disinfoper', item.id);
      }, 500);
      // this.getComponent().openPopup('disinfoper', item.id);
