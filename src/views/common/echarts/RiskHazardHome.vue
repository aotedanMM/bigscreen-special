...
import EarthDisaster from '@/components/feature/earthquake/riskHazard/EarthDisaster.feature.vue';
import {
  riskHazardServerServer,
  disasterRightEcahrs,
} from '@/api/installServer.ts';
import {
  ILatentDanger,
  IEnterprise,
  IInstallation,
} from '@/interface/feature/earthquake/EarthDisaster.interface';
@Component({
  components: {
    EarthDisaster,
  },
})
export default class RiskHazardHome extends Vue {
  private geojsonStr: any =
  ...
  private opts: any = {};
  private NewHazardQConfig: any = {}; // HazardQueryService中的 NewHazardQConfig
  private optionData = [
    {
      total: 0,
      xAxis: ['特大', '重大', '较大', '一般'],
  ...
  private allData = [
    {
      title: '地灾隐患点',
      id: 'barChart-earthDisaster',
      total: this.optionData[0].total,
      option: this.option01,
    },
    {
      title: '重点企业',
      id: 'barChart-firm',
      total: this.optionData[1].total,
      option: this.option02,
      unit: '家',
    },
    {
      title: '重点设施',
      id: 'barChart-facility',
      total: this.optionData[2].total,
      option: this.option03,
    },
  ];

  public getData() {
    this.opts.point = [116.35, 39.87];
    this.opts.geometry = JSON.parse(this.geojsonStr);
    this.opts.levelFn = this.levelFn;
    this.opts.levels = {
    ...
    this.opts.resources = this.NewHazardQConfig;
    disasterRightEcahrs.getEqStatistics(this.opts).then((res: any) => {
      console.log('ssfsfsfsfsdfsfsdfsdf', res);
    });
  }
  public levelFn(opts: any) {
    const ratio = opts.data.distance / opts.maxDistance;
    let levelCode = null;
    for (const k in opts.levels) {
