...
import { floodRiskServer } from '@/api/feature/model/installServer';
@Component({
...
export default class ResourceQueryTest extends Vue {
  ...
  private init(map: any) {
    ...
    this.commandDispatch = new FloodModel({
      map,
  ...
  private add(featureType: any) {
    this.commandDispatch.getServerData();
    const self = this;
    const opt: any = {
    ...
    this.commandDispatch._add_ImageLayer();
    this.commandDispatch._play(3);
    // this.commandDispatch.queryResource(featureType, opt);
