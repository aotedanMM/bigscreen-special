...
const component = componentBase.extend({
  options: {
    ...
    featureType: 'region_select_border',
    TooltipIdArray: [],
  },
  ...
  removeRegion(code: any) {
    const self = this;
    return new Promise((resolve, reject) => {
      const visitFn = (ele: any, layer: any) => {
        const attrs: any = Util.attributeSet2Object(ele.attributeSet);
        if (attrs.districtcode === code) {
          layer.remove(ele);
          self.toolTipWare.remove('PolygonName_' + attrs.districtname + '_' + attrs.districtcode);
          return false;
        } else {
          return true;
        }
  ...
  getGeometry() {
    ...
    const visitFn = (ele: any, layer: any) => {
      geometryArray.push(ele.geometry);
      return true;
    };
    this.simpleRenderMgr.visitFeature(this.options.featureType, {
      visit: visitFn,
    });
    const geometry = G.utils.SpatialOPUtil.unionGeometry(geometryArray);
    this._fitBounds();
    if (geometry === null) {
      return {};
    } else {
      const geojson = geometry.asGeoJson();
      const geom = turf.buffer(geojson, -0.005).geometry;
      return geom;
    }
  },
  drawRegionFromData(data: any) {
    // 绘制多边形
    this._updataPolygon([data]);
  ...
  // 更新
  _updataPolygon(dataArr: any) {
    const opt: any = {};
    opt.featureType = this.options.featureType;
    opt.list = dataArr;
  ...
  _initSimpleRenderLayer() {
    ...
    this.simpleRenderMgr.getLayer(this.options.featureType).setZIndex(8);
  },
  ...
  _showLabel(data: any) {
    ...
    const tooltip = new g2.widget.Tooltip({
      ...
      offset: [-44, 0], // 位置偏移量
      id: 'PolygonName_' + name + '_' + data.districtcode,
    });
    this.options.TooltipIdArray.push('PolygonName_' + name + '_' + data.districtcode);
