...
const component = componentBase.extend({
    ...
    load(country: any[], type: any) {
        this._showPolygon(country);
        let nonline = true;

        const ep = this.options.eventInfo.getPoint();
        if (ep) {
            this.eventPoint = [ep[0], ep[1]];
        }

        if (type === 'countyCount' && ep) {
            this._showPolygonName(country);
            nonline = false;
        }


        // this._showPolygonName(country);
        this._showDistrictPoints(country, type, nonline);
        this.addListeners();
    },
    // 卸载
    unload() {
    ...
    */
    ...
    _showPolygon(districts: any) {
        ...
        districts.forEach((item: any) => {
            const g2geom = (g2 as any).sfs.GeometryFactory.createGeometryFromWkt(item.tag.wkt);
            item.geoJson = g2geom.asGeoJson();
        });
    ...
    _showPolygonName(polygons: any) {
        for (let i = 0, len = polygons.length; i < len; i++) {
            const center = { x: polygons[i].geom.coordinates[0], y: polygons[i].geom.coordinates[1] };
            const name = polygons[i].tag.name;
            const distance = Math.round(polygons[i].tag.distance * 1);
            let contentTemplate = '<div class="district-county-box f-txt-com">' +
                ...
                '</div>';
            if (this.options.eventInfo.getType() === '10') { // 台风特殊处理不显示连线。
                contentTemplate = '<div class="district-county-box f-txt-com">' +
    ...
    _showDistrictPointsCun(data: any, type: string) {
        const symbolMapper: any = SymbolMap.DISTRICT;
        const symbolObj: any = Util.toJSON(symbolMapper.cunsymbol);
    ...
    _showDistrictPoints(data: any, type: string, noneLine: any) {
        ...
        if (point) {
            ...
            for (let i = 0, len = data.length; i < len; i++) {
                const x = data[i].geom.coordinates[0];
                const y = data[i].geom.coordinates[1];
                if (!x || x === '' || isNaN(x) || !y || y === '' || isNaN(y)) {
                    continue;
                }
                const lineCoorObj = {
                    id: i,
    ...
    _showPoints(attribute: any) {
        ...
        const SymbolBuilder = (G as any).utils.SymbolBuilder.extend({
            build: (builddata: any) => {
                // 根据数据属性控制不同的显示效果
                if (attribute[0] === 'cunCount') {
                    return G.utils.RenderUtil.object2Symbol('SimpleMarkerSymbol', attribute[5]);
                } else {
    ...
    openPopup(type: any, id: any) {
        this.closePopup();
        const typeArr: any = ['countyCount', 'townCount', 'cunCount'];
        const typeSet: any = this._getFeatureTypeSet(type);
        ...
        if (layer) {
            ...
            if (element) {
                ...
                if (type === typeArr[0]) {
                    this.options.coutypopupelement = element;
                    const borderEle: any = this.options.simpleRenderMgr.getLayer('district_county_border').find(id);
                    if (borderEle) {
                        this._hlCountyBorderAndShowTown(borderEle);
                    }
                }
                if (type === typeArr[2]) {
                    this.showHighlight(featureType, [element.geometry.x, element.geometry.y]);
                } else {
    ...
    */
    ...
    _removeDistritPoints() {
        this._removeCountyPoints();
        this._removeTownPoint();
        this.options.simpleRenderMgr.remove('district_point_cun');
    },
    _removeDistricts() {
        this.options.simpleRenderMgr.remove(this.featureTypeSet.district_county_border.type);
        this._clearDistrictHl(); // 清除高亮显示
