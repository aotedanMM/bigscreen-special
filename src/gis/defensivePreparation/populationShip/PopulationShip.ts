...
const component = componentBase.extend({
  options: {
    ...
    symbolConfig: null,
    featureType: 'populationShip_district',
    popupId: 'defensive_data_popup_id', // 弹窗唯一标识
    highLightId: 'defensive_data_hl', // 高亮id
    popupEventId: 'defensive_data_popup', // 添加弹窗后执行事件id
  ...
  locate(districtname: any) {
    ...
    if (layer.getLayerType() === 4 || layer.getLayerType() === 8) {
      for (const element of layer.elements) {
        ...
        if (attributeObj.districtname === districtname) {
          const locateObj = {
            type: 'wkt',
          ...
          this.addPopup(attributeObj, [attributeObj.longitude, attributeObj.latitude], false);
          break;
        }
      }
    }
  ...
  showDistrict(district: any) {
    ...
    const hlSymbol = (G as any).utils.RenderUtil.object2Symbol(
      'SimpleFillSymbol',
      updateSymbolObj,
    );
    const SymbolBuilder = (G as any).utils.SymbolBuilder.extend({
      build: (data: any) => {
        const textSym = new (g2 as any).sfs.TextSymbol({
          ...
          textBackgroundColor: new (g2 as any).sfs.Color({
            a: 153,
            r: 24,
            g: 62,
            b: 80,
          }),
          textBackgroundBorderColor: new (g2 as any).sfs.Color({
            a: 255,
            r: 55,
            g: 224,
            b: 245,
          }),
          foreground: new (g2 as any).sfs.Color({
            a: 255,
            r: 254,
            g: 254,
            b: 254,
    ...
    const opts = {
      ...
      listeners: {
        ...
        click: (data: any) => {
          ...
          self.addPopup(
            attributeObj,
            [attributeObj.longitude, attributeObj.latitude],
            false,
          );
        },
      },
