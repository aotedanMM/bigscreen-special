...
const component = componentBase.extend({
    ...
    showEvents(eventList: any) {
        ...
        for (const item of eventList) {
            item.typeCode = item.typeCode ? item.typeCode : EventTypeMap[item.eventType];
    ...
    locateEvent(opts: any) {
        ...
        if (geometryBuilder.check(geometry)) {
            ...
            const eventType: string = opts.typeCode ? opts.typeCode : opts.eventType;
    ...
    _showHighlight(eventType: string, coordinate: any) {
        ...
        symbolObj.options.source = this.options.symbolConfig.icons[SymbolMap.EVENT.iconHlFn(null, {
            typeCode: eventType ? eventType : EventTypeMap[eventType],
        })];
        const options = {
            data: {
                type: 'wkt',
