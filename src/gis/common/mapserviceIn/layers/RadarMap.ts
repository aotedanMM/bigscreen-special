 ...
 * 流域图层
 */
export default class RadarMap extends SimpleRenderMgrLayer {
    ...
    public _getServerData() {
        const self = this;
        this.arrData = [];
        const url = this.options.serviceConfig.floodServer + '/api/gsemergency/natural/radarAndCloudImg/listOfRadar/v1';
    ...
    public _add_ImageLayer() {
        ...
        let imageLayer = this.map.findLayer('radar_Layer');
        if (!imageLayer) {
    ...
    public _play(index: any) {
        ...
        const imageLayer = self.map.findLayer('radar_Layer');
        if (imageLayer) {
    ...
    public _clear() {
        const tempLayer = this.map.getLayerById('radar_Layer');
        if (tempLayer) {
