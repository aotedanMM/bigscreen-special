<template>
  ...
  <div class="eventInfoPop expert" ref="eventInfoPop">
    ...
    <div class="eventInfoPop_content">
      <ul>
        <el-scrollbar wrap-style="height:100%; max-height: 820px;">
          <!-- <li
            class=""
...
            />
          ...
          <li
            ...
            :key="index"
            :class="
              filterNumFixed(item, labelObj[item], list) === 'echarts'
                ? 'echartsClass'
                : ''
            "
          >
            ...
            >
              <template
                v-if="filterNumFixed(item, labelObj[item], list) === 'echarts'"
              >
...
export default class EventInfoBluePop extends Vue {
  // 要显示的按钮
  public btnFilter = [
  ...
  private filterNumFixed(key: any, label: any, list: any) {
    ...
    if (resultVal.dataType && resultVal.dataType === 'echarts') {
      return 'echarts';
    }
  ...
  private mounted() {
    ...
    if (dataDeal[that.popUpType] && dataDeal[that.popUpType].HasEcharts) {
      this.$nextTick(() => {
        // 发起房屋数据请求
  ...
  private async addEarthquakeHouseInfo() {
    const res: any = await installDisasterJudgeServer.quickJudgeServer.getEarthquakeHouseInfo(
      {
        pac: (this.data.tag && this.data.tag.adcode) || '',
      },
    );
    this.getOption(res);
  }
