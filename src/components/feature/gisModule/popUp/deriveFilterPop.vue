      ...
      <ul>
        ...
        >
          <!-- <li
            ...
            :key="key"
            v-show="getDataFilter(item.name)"
          >
            <span>{{ labelObj[item.name] }}：</span>
            <span :title="item.value + (unitObj[item.name] ? unitObj[item.name] : '')">{{removeSpaces(item.value)}} {{unitObj[item.name]}}</span>
            <img
...
            />
          ...
          <li v-for="item of dataFilter" :key="item">
            <template v-if="list[item]===0 || filterNumFixed(item, labelObj[item], list )">
              <span>{{ filterTitle(item,labelObj) }}</span>
...
export default class DeriveFilterPop extends Vue {
  // 要显示的按钮
  public btnFilter = [
  ...
  public mounted() {
    ...
    // 左侧地震事件态工贸企业数据区分显示
    if (that.popUpType === 'metallurgical' && that.event.featureType && that.event.featureType === 'disaster_judge_resource__metallurgical__0') {
    ...
    if (dataDeal[that.popUpType]) {
      ...
      that.name = dataDeal[that.popUpType].name ? dataDeal[that.popUpType].name : that.name;
...
private filterTitle(key: any, label: any) {
   const objKey = key.toLowerCase();
   const resultVal: any = label[key] ? label[key] : label[objKey];
   return resultVal;
}
  // 数字过滤
  private filterNumFixed(key: any, label: any, list: any) {
    const objKey = key.toLowerCase();
    var resultVal: any = JSON.parse(JSON.stringify(list))[key] ? JSON.parse(JSON.stringify(list))[key] : JSON.parse(JSON.stringify(list))[objKey];
    ...
    if (resultVal !== null && resultVal !== undefined) {
      ...
      if (key === 'longitude' || key === 'latitude') {
        const numbers = resultVal.trim() ? resultVal * 1 : '';
        resultVal = numbers ? numbers.toFixed(5) : '- -';
      } else {
