...
private changeTitle() {
    switch (this.codeKey) {
      case 'fireMessage':
        this.getFireBehaviorList(this.byValue);
        this.getFireBehaviorAllList(this.byValue);
        break;
      case 'histroyFire':
      this.getHistoryFireData(this.byValue);
      this.getHistoryFireAllData(this.byValue);
      default:
        break;
    }
}
  @Watch('$store.state.mapTools.showFireList', { deep: true , immediate: true})
  private initData(val: any) {
    switch (val.codeKey) {
      case 'fireMessage': // 火情信息
        this.tableConfig = [
          {
            key: 'address',
            title: '地址',
        ...
        this.getComponent()._clear('firePointToday', this.mapList);
        break;
      case 'histroyFire': // 历史火点
        this.tableConfig = [
          {
            key: 'name',
            title: '名称',
  ...
  // 获取时间
  private getNowTime(nowdate: any, type?: any) {
