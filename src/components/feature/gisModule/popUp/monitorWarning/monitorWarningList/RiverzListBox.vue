      ...
      <div class="listDistrict_body popupPanelBottom_bg" v-show="isUnfold">
        <div class="header">
          ...
          <div class="headerRightBox">
            ...
            <span class="close" @click="handleClosePops"></span>
          </div>
        </div>
...
export default class RiverzListBox extends Vue {
  ...
  private getRiverList() {
    ...
    waterSituationServer.getRiverStationsList(opts).then((res: any) => {
  ...
  private handleClosePops() {
    const params = {
      isShow: false,
      isMajorRiver: null,
      name: '',
    };
    this.$store.commit('mapTools/changeShowsRiverList', params);
  }
  ...
  private handleShowDetail(item: any) {
    this.getComponent().locate('river', 'id', item.id);
    this.isUnfold = false;
  }
  // 分页点击
