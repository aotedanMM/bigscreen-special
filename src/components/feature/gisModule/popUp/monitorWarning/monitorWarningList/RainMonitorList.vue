...
export default class RainMonitorList extends Vue {
  ...
  public getStationsList(flag: boolean = true) {
    ...
    rainSituationServer.getStationsList(opts).then((res: any) => {
      this.tableList = res.data;
      this.tableList.forEach((item: any) => {
        item.rainfall = item.rainfall ? item.rainfall + 'mm' : '0';
      });
      if (
        this.filter.level === 'rain' ||
        this.filter.level === 'heavyRain' ||
        this.filter.level === 'superRain'
      ) {
        this.tableConfig[1].title = '24H累计降水量';
      } else {
        this.tableConfig[1].title = '降水量';
      }
  ...
  private handleShowDetail(e: any) {
    // console.log(e);
    this.getComponent().locate('rain', 'id', e.id);
    this.isUnfold = false;
  }
  ...
  private updateGIS(opts: any = {}) {
    ...
    this.getComponent().addResource_Rain(opts);  // 地图上点位置
  }
}
</script>
