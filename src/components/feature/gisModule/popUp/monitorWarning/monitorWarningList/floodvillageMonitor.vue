...
export default class RiverListBox extends Vue {
  ...
  private cityCodeList: any = [
    {
      shortname: '全部市区',
      villagename: this.rootDistrictCode,
      longitude: '121.37990',
      latitude: '37.53560',
    },
  ];
  private currentCity: any = this.cityCodeList[0].shortname;
  private currentCityCode: any = this.cityCodeList[0].villagename;
  public created() {
    this.updatetype();
    this.city();
  }
  ...
  public async getStationsList(flag: boolean = true) {
    ...
    const resData: any = await waterSituationServer.getFloodvillageList(opts);
  ...
  public handleClosePop() {
    ...
    this.$store.commit('mapTools/changeShowFloodvillageList', params);
  }
  ...
  private beforeDestroy() {
    ...
    if (!this.$store.state.mapTools.showReservoirList.location) {
      this.getComponent().removeResource('floodvillage');
  ...
  private updatetype() {
    this.title = '山洪灾害村';
    this.filter.type = this.$store.state.mapTools.showReservoirList.type;
    this.filter.geometry = this.$store.state.mapTools.showReservoirList.geometry ? this.$store.state.mapTools.showReservoirList.geometry : '';
    this.nowPage = 1;
  }
  ...
  private serach(value: any) {
    if (this.filter.villagename === value) {
      return;
    }
    if (this.timer) {
  ...
  private handleShowDetail(e: any) {
    // console.log(e);
    this.getComponent().locate('floodvillage', 'id', e.id);
    this.isUnfold = false;
  }
  // 分页点击
  ...
  private updateGIS(opts: any = {}) {
    ...
    this.getComponent().removeResource('floodvillage');
    //  加载图层
    this.getComponent().addResource_Floodvillage(opts);
  }
}
</script>
