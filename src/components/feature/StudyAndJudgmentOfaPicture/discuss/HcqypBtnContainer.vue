...
@Component({
    name: 'HcqypBtnContainer',
})
export default class HcqypBtnContainer extends Vue  {
  ...
  private updateCompParam(val: any, oldVal?: any) {
    ...
    if ((val.isChecked !== oldVal.isChecked)
      ...
      this.$store.commit('mapTools/changeNearbyQueryVisible', this.curCompParam.isChecked);
    }
  }

  @Watch('$store.state.mapTools.nearbyQueryVisible')
  ...
  private getDefaultCheckedState() {
    // 其实这个按钮的高亮情况只有一种，就是主动点击（需求说的）和正处于研判
    // 在组件初始化的时候，是不用关心和父级和同级兄弟按钮之间的关系的
    // 可以使得这个按钮高亮的情况有，缓冲区面板出现
    // zhypGeoType.key === hcqyp
    const hcqmbIsShow = this.$store.state.mapTools.nearbyQueryVisible; // 缓冲区面板是否显示
    const val = this.$store.state.dataFilterControl.zhypGeoType;
    const isCheckedKey = val && val.key && (val.key === this.curCompParam.key); // 当前地图上是否有缓冲区的研判
    if (isCheckedKey && hcqmbIsShow) { // 表示处于缓冲区研判研判中
      this.curCompParam.isChecked = true;
    }
  }
