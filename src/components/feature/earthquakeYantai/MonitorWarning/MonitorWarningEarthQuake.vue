...
import EarthWeatherForewarning from './EarthWeatherForewarning.vue';
 ...
 */
@Component({
  name: 'MonitorWarningEarthQuake',
  components: {
    RainMonitor,
    ZoomBtn,
    EarthWeatherForewarning,
  },
  mixins: [MapCommon],
})
export default class MonitorWarningEarthQuake extends Vue {
  ...
  private initData() {
    monitorWarningServer.getWarningStatistics({}).then((result: any) => {
      ...
      console.log(data, 'datayujing');
      const keyMap: any = {
        RainMonitor: 'rainMonitorNum',
        EarthWeatherForewarning: 'earthWeatherWarningNum',
      };
  ...
  private mounted() {
    ...
    this.getComponent().on(
      'weatherWarningLayer_popup',
      this.popupDataFireWeatherForewarning,
      this,
    );
    this.addlayerMap();
