...
export default class GisPlot extends Vue {
  ...
  private mapOpts: any = {
    businessId: '2f2ee16f-44ac-c7ed-c8ba-52ec60a1a1fe', // '2f2ee16f-44ac-c7ed-c8ba-52ec60a1a1fe',
  };
  ...
  public plotClose(isClose: boolean = false) {
    ...
    if (isClose) { // 如果是点击的关闭按钮
      this.messageBus.emit('commonTools', 'realTimePlotting' , isClose);
    } else {
  ...
  private toTabItem(item: any, index: any) {
    ...
    } else if (item.value === 'basic') {
       (this.$refs as any).gisPanelBasic.refreshLocalPic();
    } else if (item.value === 'common') {
       (this.$refs as any).gisPanelHottab.refreshLocalPic();
    }
  }
  // 点击保存标绘按钮
  ...
  private businessPlotSave(opts: any) {
    ...
    // opts.page = 1;
    // opts.pageSize = 5,
    this.mapComponent.savePlotSchema(opts.businessId, opts.name, opts.page, opts.pageSize);
  }
  /**
   * 外部标绘方案列表查询
   * @param opts
  */
  private businessPlotList(opts: any) {
    const self = this;
    this.plotInit().then(() => {
      this.mapComponent.refreshPlotSchemaList(opts.businessId, opts.page, opts.pageSize, '');
    });
  }
  /**
   * 外部标绘方案列表 关键字查询
  ...
  */
  private businessPlotListSearch(opts: any) {
    const self = this;
    this.mapComponent.planSearchBack(opts.businessId, opts.name, opts.page, opts.pageSize);
  }
  /**
   * 外部标绘方案加载
   * @param opts
  ...
  */
  private businessPlotSchemaDelete(opts: any) {
    const self = this;
    this.mapComponent.planDeleteBack(opts.businessId, opts.schemaId, opts.page, opts.pageSize);
  }
  /**
   * 暴雨落区 标绘 进入处置
   * @param opts
  */
  private businessPlotStartDisposal(opts: any) {
    ...
    if (opts.schemaId) { // 已保存过的
      ...
      geometry = geometry.asGeoJson();
      if (opts.flag) { // 自动保存
        this.mapComponent.savePlotSchema(opts.businessId, opts.name, opts.page, opts.pageSize, true);
      }
      // console.debug('businessPlotDisposalReturn', {geometry});
      this.messsageBus.emit('businessPlotDisposalReturn', {
        geometry,
  ...
  */
  ...
  private setDistrictCode() {
    if (window.sessionStorage.districtCode) {
      this.mapOpts.businessId = window.sessionStorage.districtCode;
    }
  }
  // messsageBus的监听
  private init() {
  ...
  private initTabContent() {
    ...
    // 1常用标绘获取json初始化方法
    (this.$refs as any).gisPanelHottab.gethotDatas(this.eventType);

    // this.gethotDatas();
    // 2标绘工具箱获取json初始化方法
    // this.getChinaTabNames();
    (this.$refs as any).gisPanelBasic.getChinaTabNames(this.eventType);

    // 获取应急标绘符号json
    // this.getPlotSymbol();
    (this.$refs as any).gisPanelEmergency.getPlotSymbol(this.eventType);
  }
}
</script>
<style lang="less">
  ...
  .el-pagination{
    ...
    .el-pagination__totalm, .pagination__jump,input, span, button{
      color: #ffffff!important;
  ...
  .el-loading-mask{
    background-color: rgba(7,16,34,.8) !important;
  }}
</style>
