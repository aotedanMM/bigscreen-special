...
export class PlotManagerMapComponent extends window.G.base.ComponentBase {
        ...
        switch (this.mapDemension) {
            case '2d':
                if (this.plotComponent2D) {
                    ...
                    // 创建地图监听
                    this.plotComponent2D.off(window.G.misc.AppEvents.PLOT_CLICKED, this._plotClickListener, this);
                    this.plotComponent2D.on(window.G.misc.AppEvents.PLOT_CLICKED, this._plotClickListener, this);
                    // 标绘完成
                    this.plotComponent2D.off(window.G.misc.AppEvents.PLOT_CREATED, this._plotCompleteListener, this);
                    this.plotComponent2D.on(window.G.misc.AppEvents.PLOT_CREATED, this._plotCompleteListener, this);
                    this.plotComponent2D.off('openTailedLabelEdit', this._openTailedLabelEdit, this);
    ...
    */
    ...
    public _plotClickListener(event: any) {
        ...
        if (plotElement != null) {
            if (this._plotEffective(plotElement)) {
                ...
                const businessId = plotElement.tag.businessId ? plotElement.tag.businessId : '2f2ee16f-44ac-c7ed-c8ba-52ec60a1a1fe';
    ...
    // 标绘编辑
    public editSchema(businessId: string, opts: any) {
        const self = this;
        const options: any = {};
        options.id = opts.id;
    ...
    public clearResults() {
        ...
        this.getPlotComponent()._plotOverlayTool.overlayWare.clear();
    ...
    public removeControl() {
        this.clearResults();
        if (this.plotLayer) {
            this.options.map.removeLayer(this.plotLayer);
        }
    }
    /**
     * 根据businessId移除标绘图形
