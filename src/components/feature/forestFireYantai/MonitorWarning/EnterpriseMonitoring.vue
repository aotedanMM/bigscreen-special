<template>
  ...
  <div class="FirePointMonitorBox">
    <div class="title">
      <span class="title-panel">企业监测</span>
    </div>
...
export default class EnterpriseMonitoring extends Vue {
  ...
  private curcontList: any = [
    {
      title: '今日报警企业',
      sum: 0,
      unit: '起', // 经兴民总确认，火点单位统一为个
      active: false,
      showPopList: 'hqxx',
      isShowOpen: false,
      list: [],
      codeKey: 'histro1yFire',
      state: '1',
    },
    {
      title: '历史报警企业',
  ...
  // 企业报警接口数据
  private async indexAjax() {
    const res: any = await firePointMonitorServer.enterpriseAlarm();
    if (res.status === 200) {
      // 今日报警
      this.curcontList[0].sum = res.data.today;
      // 月报警
      this.curcontList[1].sum = res.data.history;
    }
    console.log(res);
  }
