...
@Component({
  name: 'GisPlotControlBtn',
})
export default class GisPlotControlBtn extends Vue {
  ...
  private clickHandler(item: any) {
    ...
    obj.forEach((item1: any) => {
      if (tempFun[item1.key]) {
        tempFun[item1.key](this, item1);
      }
    });
  ...
  private initListener() {
    this.messsageBus.off('GisPlotControlBtn', this.handleCommonToolsClose);
    this.messsageBus.on('GisPlotControlBtn', this.handleCommonToolsClose);
  }
