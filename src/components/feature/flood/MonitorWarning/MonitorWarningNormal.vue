<template>
  ...
  <div>
    <div v-show="!collapsed" class="monitor-warning-panel">
      <!-- <div
        class="monitor-warning-btn monitor-warning-btn--close"
        @click="collapse"
      ></div> -->
      <!---导航面板--->
          ...
          <div class="monitor-tab">
            ...
            >
              ...
              <div class="monitor-tab_itm_txt" @click="tabClick(item)">
                ...
                <span class="inner f-txt-small" v-if="item.label !== '台风监测'"
                  >共
                  <b
                  ...
                  >
                  {{
                    item.isWarning === 'EngineeringMonitor'
                      ? '处监测站点'
                      : item.isWarning === 'WeatherForewarning'
                      ? '处预警信息'
                      : '处告警站点'
                  }}</span
                >
                <span class="inner f-txt-small" v-else
                  >当前<b class="f-number txt-warn">{{
                    item.isWarning === 'dqtf' ? '有' : '无'
                  }}</b>
                  台风影响
                </span>
              </div>
    ...
    ></div>
    <!-- 弹窗暂时展示 -->
    <!-- <component class="popup" v-if="currentTab === 'WaterMonitor' || currentTab === 'WindMonitor' || currentTab === 'RainMonitor' || currentTab === 'EngineeringMonitor'" :is="currentTab + 'Popup'"></component> -->
  </div>
</template>
...
import WindMonitor from './WindMonitor.vue';
import RainMonitor from './RainMonitor.vue';
import EngineeringMonitor from './EngineeringMonitor.vue';
import VideoMonitor from './VideoMonitor.vue';
import WeatherForewarning from './WeatherForewarning.vue';
import typhoonView from './typhoonView.vue';
import MapCommon from '@/util/MapCommon';
import publishObjectPath from '@/util/configRegistry';
import renderpopUpTemplate from '@/components/feature/gisModule/popUp/renderpopUpTemplate.vue';
import ReservoirMonitor from './ReservoirMonitor.vue'; // 水库
import RiverMonitor from './RiverMonitor.vue'; // 河流
...
export default class MonitorWarningNormal extends Vue {
  private isShowModal: any = true;
  ...
  private monitorTabs: any = [
    ...
    // },
    {
      label: '水库监测', // reservoir   river
      value: 'ReservoirMonitor',
      count: 0,
      icon: 'reservoir',
      class: {
        'monitor-tab_itm': true,
    ...
    {
      label: '工情监测',
      value: 'EngineeringMonitor',
      isWarning: 'EngineeringMonitor',
      count: 0,
      icon: 'working',
      class: {
        'monitor-tab_itm': true,
  ...
  private initData() {
    monitorWarningServer.getWarningStatistics({}).then((result: any) => {
      ...
      const keyMap: any = {
        ...
        WaterMonitor: 'waterWarningNum',
        EngineeringMonitor: 'engineeringNum',
        VideoMonitor: 'cameraNum',
        WeatherForewarning: 'weatherWarningNum',
        RiverMonitor: 'riverWarningNum',
        ReservoirMonitor: 'reservoirWarningNum',
      };
  ...
  private addlayerMap() {
    (this as any).resolveMap('map').then((map: any) => {
      ...
      this.addLayer(this.monitorTabs[1]);
      this.addLayer(this.monitorTabs[2]);
      this.addLayer(this.monitorTabs[3]);
      this.addLayer(this.monitorTabs[4]);
    });
  }
  private clearLegend() {
  ...
  private addLayer(item: any) {
    ...
    switch (item.label) {
      ...
      case '风情监测':
        ...
        break;
      case '水库监测':
        this.getComponent().addResource_Water_reservoir();
        break;
      case '河流监测':
        this.getComponent().addResource_Water_river();
        break;
      case '雨情监测':
  ...
  private async getCountData() {
    const opts = {
      type: '11B06,11B03,11B01,11B14,11B37',
      startTime: getDateFormat({ last: 'year' }), //  oneDay
      endTime: getDateFormat(),
      districtCode: publishObjectPath.value.district.root,
      searchType: '1',
    };
      ...
      Object.keys(res.data).forEach((item: any) => {
        if (item === '11B01') {
          if (res.data[item].length > 0) {
            Object.keys(self.monitorTabs).forEach(
              (results: any, index: any) => {
                if (self.monitorTabs[index].label === '台风监测') {
                  self.monitorTabs[index].isWarning = 'dqtf';
                }
              },
            );
            // for (let index = 0; index < self.monitorTabs.length; index++) {
            //   if (self.monitorTabs[index].label === '台风监测') {
            //     self.monitorTabs[index].isWarning = 'dqtf';
            //   }
            // }
  ...
  private downloadFile() {
    monitorWarningServer
      ...
      .then((res: any) => {
        ...
        dom.download =
          '总体预警报告' + this.dateFormat('YYYYmmdd', new Date()) + '.docx';
...
.monitor-tab {
  &_itm {
    ...
    &:hover {
      ...
      /*水库*/
      .monitorTab-icon-reservoir {
        background: url('@{pictureUrl}/reservoirHover.png') -18px 0;
      }
      /*河流*/
      .monitorTab-icon-river {
        background: url('@{pictureUrl}/riverHover.png') -18px 0;
      }
      /*台风*/
      .monitorTab-icon-typhoon {
        background: url('@{pictureUrl}/typhoonHover.png') -18px 0;
      }
...
/*水库*/
.monitorTab-icon-reservoir {
  background: url('@{pictureUrl}/reservoir.png') -18px 0;
}
/*河流*/
.monitorTab-icon-river {
  background: url('@{pictureUrl}/river.png') -18px 0;
}
/*水情*/
...
/*台风*/
.monitorTab-icon-typhoon {
  background: url('@{pictureUrl}/typhoon.png') -18px 0;
}
/*工情*/
...
.reportIcon {
  ...
  width: 38px;
  height: 38px;
  background: url('../../../../assets/img/monitorWarning/reportIcon.png')
    no-repeat 0 0;
  ...
  margin-top: 2px;
  &:hover {
    background: url('../../../../assets/img/monitorWarning/reportIcon_hover.png')
      no-repeat 0 0;
    background-size: 100% 100%;
  }
}
