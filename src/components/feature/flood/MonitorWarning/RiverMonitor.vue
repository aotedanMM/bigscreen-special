<template>
  ...
  <div class="panelPublicDefault water-monitor-panel">
    <div class="panelPublicDefault_hd">
      <!-- ({{ count }}) -->
      <span class="title-panel">河流监测</span>
    </div>
    ...
    <div class="panelPublicDefault_bd">
      ...
      <div class="statisticList">
        ...
        <div
          ...
          @click="changeTab(1, 'warning')"
        >
          <span>告警监测站</span>
          <div>
            <span class="statisticList_li_textWarning f-number">{{
              countData.warningNum || 0
            }}</span
            >处
          </div>
        </div>
      ...
      <!-- 搜索 end-->
      <div class="list" :class="{ listLeft: !isShowModal }">
        <div
          class="nodata"
...
export default class RiverMonitor extends Vue {
  // 接收 检测总数
  @Prop() private count!: number;
  // 是否显示echartModal
  @Prop() private isShowModal: any;
  // 预警信息报告弹窗
  private title: any = '河道监测预警';
  private lineHead = [
    { name: '超警戒水位站点：', value: '0', unit: '处', key: 'alertNum' },
    { name: '超保证水位站点：', value: '0', unit: '处', key: '' },
  ...
  private created() {
    this.getWaterLevel();
    this.showJsl('Riverway');
    this.getMaxWaterInfo();
    this.changeGeometry(); // 监听geometry
  }
  ...
  private beforeDestroy() {
    ...
    this.$store.commit('mapTools/removeSelectedLayer', {id: 'Riverway'});
  ...
  private async getCountData() {
    ...
    // const obj = {
    //   geometry: this.searchData.geometry,
    // };
    const res: any = await waterSituationServer.getCurrentRiverInfo(
  ...
  private async getWaterLevel() {
    const res: any = await waterSituationServer.getRiverRiseAndRetreat({});
    this.pieData = [
  ...
  private async getSiteList(flag: boolean = true) {
    const res: any = await waterSituationServer.getRiverStationsList(
      this.searchData,
    );
    ...
    if (flag) {
      this.updateGIS({
        ...
        districtCode: this.searchData.districtCode === '130400' ? '' : this.searchData.districtCode,
  ...
  private openProp(id: any) {
    this.activeIndex = id;
    this.getComponent().locate('river', 'id', id);
  }
  // 打开预警报告弹窗 列表
  ...
  private async openDialogWarning() {
    ...
    const statRes: any = await waterSituationServer.getWarningStatInfo();
  ...
  private city() {
    ...
    nomalLeftServer.getCitySelected().then((res: any) => {
      ...
      if (self.$store.state.dataFilterControl.filter.districtCode === '370600') {
        return;
      }
  ...
  private changeGeometry() {
    if (this.isShowModal === false) {
      ...
      this.$store.state.dataFilterControl.filter.districtCode === '130400'
          ...
          : this.$store.state.dataFilterControl.filter.districtCode;
      if (this.$store.state.dataFilterControl.filter.geometry) {
        this.searchData.geometry = this.$store.state.dataFilterControl.filter.geometry;
      } else {
        this.searchData.geometry = '';
      }
      this.city();
      this.getCountData(); // 面板统计
      this.getSiteList(); // 列表
    }
    if (this.$store.state.dataFilterControl.filter.geometry) {
...
@imgPath1: '../../../../assets/img/monitorWarning';
.water-monitor-panel {
  .el-scrollbar__thumb {
