...
<div>
    ...
    <div class="right_side_function"  v-show="smallButtonBoxShow">
        <div class="right_function_box right_function_box_w" v-show='this.checkedNum!==0 '>
        ...
        <div class="right_sidebar">
            <el-scrollbar style="height:calc(100% - 120px)">
                ...
                <div class="right_sidebar_itme" @click="changeChecked(item,index)" v-for="(item,index) of arrFromConfigNew"
                    :key="item.key"
                    :class="{testChecked:item.checked}">
...
export default class FunToolHalf extends Vue {
    ...
    private rightPanelShowRight = {
        ...
        cityListLocation: 'right: 5px;', // 城市列表的位置
        peripheralQueryLocation: 'right:85px;', // 周边查询面板的位置
    ...
    private rightPanelShowCenter = {
        ...
        cityListLocation: 'right: 65px;',
        peripheralQueryLocation: 'right:85px;',
        isShowquanjing: true,
        RegionSelection: 'right: 130px;', // 图层选择的面板的位置
    };
    private rightPanelShowLeft = {
        ToolbarLocation: 'right: 460px;',
        experienceCirclePosition: 'right: 37.5%',
        cityListLocation: 'right: 474px;',
        peripheralQueryLocation: 'right:490px;',
        isShowquanjing: false,
        RegionSelection: 'right: 474px;', // 图层选择的面板的位置
    };
    // 判断当前点击的是哪一个key
    private changeChecked(item: any, index: number) {
        ...
        if (this.smallButtonBoxShow) {
            // 当面板存在时顶部右侧gis工具容器
            this.$store.commit('panelPositionChangeModule/settopToolbarLocation', this.rightPanelShowLeft );
        }
        if (item.checked === true) {
            ...
            if (this.checkedNum === 0) {
                ...
                this.$store.commit('panelPositionChangeModule/settopToolbarLocation', this.rightPanelShowCenter );
            }
        ...
        if (this.arrFromConfigNew.findIndex((target: any) => target.checked === true ) === -1) {
   ...
   // 更新图例的位置。
    private updatedLengedLoca() {
        ...
        if (this.checkedNum < 3 && !this.enlargeHeight) {
            this.$store.commit('panelPositionChangeModule/setbotLegendLocation', 'right: 135px;' );
            this.$store.commit('panelPositionChangeModule/setrightPanelPosition', this.leftPanelShowRight );
        } else if (this.smallButtonBoxShow || this.checkedNum === 3 || this.enlargeHeight) {
            this.changeLegendLocation(!this.smallButtonBoxShow);
            this.$store.commit('panelPositionChangeModule/setrightPanelPosition', this.leftPanelShowLeft );
        }
    }
    ...
    private smallButtonBox() {
        ...
        if ( this.smallButtonBoxShow === true) {
            ...
            this.$store.commit('panelMutualExclusionMudule/setpanelMutualExclusion', {largeRightPanel: {showFlag: true}} );
            ...
            this.$store.commit('panelMutualExclusionMudule/setpanelMutualExclusion', {largeRightPanel: {showFlag: false}} );
    ...
    private arrFromConfigNewFun(val: any) {
        ...
        const checkedFilter = this.arrFromConfigNew.filter((item: any, index: number) => {
            return item.checked;
        });
    ...
    private changeLegendLocation(data: any) {
        if (data) {
            ...
            this.$store.commit('panelPositionChangeModule/setbotLegendLocation', 'right: 540px;' );
        }
    }
    ...
    private changelargeRightPanel(val: any) {
        ...
        if (this.smallButtonBoxShow) {
            ...
            this.$store.commit('panelPositionChangeModule/settopToolbarLocation', this.rightPanelShowRight );
        }
    }
    ...
    private created() {
        this.changelargeRightPanel(this.$store.state.panelMutualExclusionMudule.panelMutualExclusion); // 监听当前面板的显隐
...
.rightPanelShrink-btn.reserve{
 ...
 .right_side_function{
    ...
    .right_function_box_shink{
        ...
        z-index: 1;
        pointer-events: auto;
        outline:10px solid red;display:inline-block;z-index:4;right:0;
    }
