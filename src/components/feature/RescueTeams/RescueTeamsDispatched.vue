      ...
      <div class="rescueTeamsHome_cnt" v-if="!loading">
        <div class="rescueTeamItem" v-for="(item, index) of data" :key="index">
          <div class="tempRight-title  f-tit-h2">
            <!--只有当前选中且数据大于0的时候才会高亮-->
            <span :title="item.orgName ? item.orgName : '暂无数据'">
              {{item.orgName ? item.orgName : '暂无数据'}}
            </span>
          ...
          <template v-if="item.showSub">
            <div class="tempRight-cont">
              <div class="teamConfiguration">
                <div><span>出动人数:</span><span>{{item.dispatchNum ? item.dispatchNum + ' 人' : '暂无数据'}}</span></div>
                <div><span>带队领导:</span><span>{{item.leaderName ? item.leaderName : '暂无数据'}}</span></div>
                <div><span>联系电话:</span><span>{{item.conactTel ? item.conactTel : '暂无数据'}}</span></div>
                <div><span>出动车辆:</span><span>{{item.carNum ? item.carNum : '暂无数据'}}</span></div>
                <div>
                  <span>携带装备:</span>
                  <span v-if="!item.equipliststr || !item.equipliststr.length">暂无数据</span>
                  <span v-else v-for="(k, v) in item.equipliststr" :key="v">{{k.equipName ? k.equipName : '暂无数据'}}</span>
                </div>
                <div><span>当前位置：</span><span>{{item.address ? item.address : '暂无数据'}}</span></div>
              </div>
              <div class="journey">
...
@Component({
  name: 'RescueTeamsDispatched',
  components: {
    // VideoCallPop,
  },
})
export default class RescueTeamsDispatched extends Vue {
  ...
  private created() {
    ...
    console.log(this.rescueTeamHomeData, 'data');
  }
  private mounted() {
    // this.getComponent().ShowRealTimeTeamRoute({teamId: this.rescueTeamHomeData.parameter.teamId});
  ...
  private dispatchTeamList() {
    if (!!this.rescueTeamHomeData.TeamsDispatchedData) {
      ...
      const obj = {
        ...
        teamId: this.rescueTeamHomeData.parameter.teamId,
        endPoints: [this.$store.state.eventPushStore.eventLocation.EventLon, this.$store.state.eventPushStore.eventLocation.EventLat],
      };
      const promise1 = this.dispatchServer.dispatchTeamList(
        obj,
      );
      const promise2 = this.getComponent().ShowRealTimeTeamRoute({
        teamId: this.rescueTeamHomeData.parameter.teamId,
      });
      // Promise.all([promise1, promise2]).then((res: any) => {
      //     console.log('hhh', res);
      //     this.journeyList = res[1];
      //     this.data = res[0].data.list;
      //     this.data.teamId = this.rescueTeamHomeData.parameter.teamId;
      //     this.loading = false;
      // });
      promise1.then((res1: any) => {
        ...
        promise2.then((res2: any) => {
          this.journeyList = res2;
        });
      });
    }
  }
  ...
  private openFeedback(item: any) {
    ...
    // this.getComponent().clearRealTimeLayer();
    // this.getComponent().unload();
    this.rescueTeamHomeData.TeamsDispatchedData = this.data; // 保存队伍出动情况的数据
