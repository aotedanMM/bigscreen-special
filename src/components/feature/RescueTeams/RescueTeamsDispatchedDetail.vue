  ...
  <div class="rescueTeamsHome">
    ...
    <el-scrollbar>
      ...
      <div class="teamConfiguration">
        <div><span>出动人数:</span><span>{{detailsData.dispatchNum ? detailsData.dispatchNum + ' 人' : '暂无数据'}}</span></div>
        <div><span>带队领导:</span><span>{{detailsData.leaderName ? detailsData.leaderName : '暂无数据'}}</span></div>
        <div><span>联系电话:</span><span>{{detailsData.conactTel ? detailsData.conactTel : '暂无数据'}}</span></div>
        <div><span>出动车辆:</span><span>{{detailsData.carNum ? detailsData.carNum : '暂无数据'}}</span></div>
        <div>
          <span>携带装备:</span>
          <span v-if="!detailsData.equipliststr.length">暂无数据</span>
          <span v-else v-for="(k, v) in detailsData.equipliststr" :key="v">{{k.equipName ? k.equipName : '暂无数据'}}</span>
        </div>
        <div><span>当前位置:</span><span>{{detailsData.address ? detailsData.address : '暂无数据'}}</span></div>
        <div class="journey">
