...
<template>
  <div class="rescueTeamsHome halflist">
    ...
    <el-scrollbar v-if="feedbackData || demandData">
      <div
        class="detachment_name"
        ref="detachment_name"
      >{{rescueTeamHomeData.parameter.orgName || '暂无数据'}}</div>
      <div class="rescueTeamsHome_cnt">
        <div class="rescueTeamItem" ref="feedbackBoxTop">
          <div class="tempRight-title f-tit-h2">
            <span>任务反馈</span>
            <i
              :class="feedbackShow? 'tempRight-switch':'tempRight-switch tempRight-switch-reverse'"
              @click.stop="feedbackBoxShow()"
            ></i>
          </div>
            ...
            <div class="tempRight-cont feedbackBox">
              <el-scrollbar :wrap-style="'max-height: 330px;'">
                <div v-for="(item, index) in feedbackData" :key="index">
                  <p>
                    反馈：
                    <span :title="item.backinfo">{{item.backinfo ? item.backinfo : '暂无数据'}}</span>
                  </p>
                  <p>
                    任务：
                    <span :title="item.tasktitle">{{item.tasktitle ? item.tasktitle : '暂无数据'}}</span>
                  </p>
        ...
        <div class="rescueTeamItem">
          <div class="tempRight-title f-tit-h2">
            <span>需求提报</span>
            <i
              :class="demandShow? 'tempRight-switch':'tempRight-switch tempRight-switch-reverse'"
              @click.stop="demandShow = !demandShow"
            ></i>
          </div>
            ...
            <div class="tempRight-cont demandBox" :style="{height: listHeight + 'px'}">
              <el-scrollbar style="height: 100%">
                <div v-for="(item, index) in demandData" :key="index">
                  <span>{{index + 1}}</span>
                  <p>{{item.information ? item.information : '暂无数据'}}</p>
                  <p>{{item.createtime ? item.createtime : '暂无数据'}}</p>
                  <p
                    class="type"
                    :class="{'rescue' : item.demandflag === '1'}"
                  >{{item.demandflag === '1' ? '救援需求' : '物资需求'}}</p>
                </div>
              </el-scrollbar>
            </div>
          </template>
...
export default class SiteFeedback extends Vue {
  ...
  private async getDispatchtaskback() {
    ...
    const res: any = await installDisasterJudgeServer.rescueTeamServer.getDispatchtaskback(
      obj,
    );
      ...
      this.feedbackData.forEach((item: any, index: number) => {
        item.attachmentList.forEach((v: any) => {
          ...
          this.$nextTick(() => {
            this.listHeight =
              740 -
              (this.$refs.feedbackBoxTop as HTMLElement).offsetHeight -
              (this.$refs.detachment_name as HTMLElement).offsetHeight;
          });
        });
      });
    }
  ...
  private async getQueryPresentationList() {
    ...
    const res: any = await installDisasterJudgeServer.rescueTeamServer.getQueryPresentationList(
      obj,
    );
