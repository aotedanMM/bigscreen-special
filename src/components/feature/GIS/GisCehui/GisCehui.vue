<template>
    <div>
        <div id="buttons" class="cehuimain">
            <span>
                ...
                <input type="button" value="测量圆" id="measureCircle" @click="allTestFunc('measureCircle')">
                <input type="button" value="测量椭圆" id="measureEllipse" @click="allTestFunc('measureEllipse')">
...
export default class GisCehui extends Vue {
    ...
    private mounted() {
        // 获取地图，地图加载完成后进入回调
        const self: any = this;
        self.resolveMap(self.options.mapId).then((event: any) => {
    ...
    private allTestFunc(type: any) {
        ...
        switch (type) {
            ...
            case 'measurepolygon':
                ...
                break;
            case 'measureCircle':
                this.measureComponent.measureCircle();
                break;
            case 'measureEllipse':
                this.measureComponent.measureEllipse();
                break;
            case 'finish':
    ...
    private initMeasure(map?: any) {
        ...
        // console.debug('>> 测量地图ID', map.options.targetId);
    }
