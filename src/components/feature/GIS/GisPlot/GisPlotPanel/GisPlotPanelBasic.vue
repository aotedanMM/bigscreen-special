...
export default class GisPlotPanelBasic extends Vue {
  ...
  private comTitleClick(item: any, index: any) {
   ...
   // 请求标绘工具箱数据;
  private getCommonTabDatas(nameObj: any) {
    ...
    return new Promise((resolve, reject) => {
      ...
      plotServer
        ...
        .then((res: any) => {
          // const temp = res.data ;
          const temp = res;
          this.getCommonTabDatas(temp.normalPlot);
          resolve();
        });
    });
  }
  ...
  private LocalPic(val: any, ele: any) {
    // 判断是否已经存在同名的图标
    if (this.localJson.some((item: any) => item.name === ele.target.files[0].name)) {
      ele.currentTarget.value = '';
      return this.$message('图标已存在, 请重新选择');
    }
