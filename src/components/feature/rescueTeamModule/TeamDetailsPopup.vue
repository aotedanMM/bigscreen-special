    ...
    <div class="newDetailsProtrusion-baseinfo-content">
      ...
      <el-scrollbar class="newDetailsProtrusion-baseinfo-content-top">
        ...
        <p>
          ...
          <span class="TeamLeix">{{rescueTeamHomeData.captain ? rescueTeamHomeData.captain : '- -'}}</span>
...
export default class TeamDetailsPopup extends Vue {
  ...
  private async initMapEvents() {
    ...
    } else if (this.$store.state.eventPushStore.eventLocation.EventLat && this.$store.state.eventPushStore.eventLocation.EventLaton) {
      startAddress = {
  ...
  @Watch('rescueTeamHomeData')
  private ChangeRescueTeamHomeData() {
    this.init();
  }
  private init() {
    const self: any = this;
