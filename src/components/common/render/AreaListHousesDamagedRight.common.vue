    ...
    <ul class="listBox" v-if="!noData" >
      ...
      >
        ...
        <span
          ...
          @mouseleave="itemiMouout"
          :class="{ indexSpan: item === 'num' }"
          v-show="item === 'seriousDamage'"
        >
          {{ each[item] | FiltersHouse }}
        </span>
        <!-- 严重受损开始结束 -->
        <!-- 一般受损开始开始 -->
        <span
...
@Component({
  ...
  filters: {
    // 过滤房屋受损
    FiltersHouse(val: any): any {
      if (val === '' || val === undefined || val === null || val === 0) {
        return '暂无数据';
      } else {
        return Math.floor(val * 100) / 100 + '万间';
      }
    },
    // 过滤距事发地距离
