    ...
    <ul class="listBox" v-if="!noData"> 
      ...
      >
        ...
        <span
          ...
          @mouseleave="itemiMouout"
          :class="{ indexSpan: item === 'num' }"
          v-show="item === 'lineDamage'"
        >
          {{ each[item] | FiltersLine }}
        </span>
        <!-- 受损线路开始结束 -->
        <!-- 受损变电站开始开始 -->
        <span
          ...
          @mouseleave="itemiMouout"
          :class="{ indexSpan: item === 'num' }"
          v-show="item === 'transformerSubstationDamage'"
        >
          {{ each[item] | FilterTransformTerSubstation }}
        </span>
        <!-- 受损变电站结束 -->
        <!-- 距事发地开始 -->
        <span
...
@Component({
  ...
  filters: {
    // 受损线路
    FiltersLine(val: any): any {
      if (val === '' || val === undefined || val === null || val === 0) {
        return '暂无数据';
      } else {
        return Math.floor(val * 100) / 100 + '条';
      }
    },
    // 过滤距事发地距离
    ...
    // 过滤变电站
    FilterTransformTerSubstation(val: any): any {
      if (val === '' || val === undefined || val === null || val === 0) {
        return '暂无数据';
      } else {
        return val + '座';
...
export default class AreaList extends Vue {
  ...
  private FnSelectSchoolData(): void {
    ...
    console.log('当前分类下拉框的值：');
    console.log(typeof(this.selectSchoolClass));
    for (const iterator of this.listData) {
    ...
    console.log('当前重组后的selectSchoolClassData:');
    console.log(this.selectSchoolClassData);
  }
