    ...
    <ul class="listBox" v-if="!noData">
      ...
      >
        ...
        <span
          ...
          @mouseleave="itemiMouout"
          :class="{ indexSpan: item === 'num' }"
          v-show="item === 'backToBaseStation'"
        >
          {{ each[item] | FilterBackToBaseStation }}
        </span>
        <!-- 退服基站开始结束 -->
        <!-- 恢复数量开始开始 -->
        <span
  ...
  filters: {
    ...
    FiltersLine(val: any): any {
      if (val === '' || val === undefined || val === null || val === 0) {
        ...
        return Math.floor(val * 100) / 100 + '个';
      }
    },
    // 过滤距事发地距离
    ...
    // 过滤退服基站
    FilterBackToBaseStation(val: any): any {
      if (val === '' || val === undefined || val === null || val === 0) {
        return '暂无数据';
      } else {
        return val + '座';
